...
This file is the exact claim boundary for NotesRecover's public distribution
story.
...
| Codex | yes, the official Codex plugin directory exists, but third-party official-directory submission is still coming soon | yes: `plugins/notes-recover-codex-plugin/` | companion plugin lane | repo-owned Codex bundle shipped; do not claim official Codex directory listing |
| Claude Code | yes, official plugin and marketplace surfaces exist | yes: `plugins/notes-recover-claude-plugin/` plus root `.claude-plugin/marketplace.json` | companion plugin lane | repo-owned Claude plugin and marketplace metadata shipped; do not claim Anthropic-managed listing without fresh read-back |
| OpenClaw | yes, the official ClawHub public registry exists | yes: `plugins/notes-recover-openclaw-bundle/` | comparison-path companion lane | the secondary ClawHub public-skill listing is live, but the shipped OpenClaw-compatible bundle still does not prove an official OpenClaw listing or first-class host wrapper acceptance |
| OpenHands/extensions | yes, the official OpenHands public extensions registry exists | yes: `public-skills/notes-recover-case-review/` plus canonical `skills/notes-recover-case-review/` | portable public skill lane | public skill folder shipped; the current thread is submission-done plus changes-requested, so do not claim a live OpenHands/extensions listing until maintainer acceptance lands |
...
- `plugins/notes-recover-codex-plugin/`
- `plugins/notes-recover-claude-plugin/`
- `plugins/notes-recover-openclaw-bundle/`
- `skills/notes-recover-case-review/`
- `public-skills/notes-recover-case-review/`
...
- canonical truth still lives in `skills/notes-recover-case-review/`
...
- this is the intended PyPI package identifier and version for the current repo
  contract: `notes-recover==0.1.0.post1`
...
- `ghcr.io/xiaojiou176-open/notes-recover:0.1.0.post1`
- optional convenience tag: `ghcr.io/xiaojiou176-open/notes-recover:latest`
...
```bash
claude plugin validate plugins/notes-recover-claude-plugin
...
./.venv/bin/python scripts/release/build_starter_bundles_bundle.py --out ./dist/notes-recover-host-starters.zip
```
...
- "`server.json` captures the current stdio-first MCP descriptor for NotesRecover"
