...
def test_auto_run_pipeline_e2e(tmp_path: Path, monkeypatch) -> None:
    ...
    (spotlight / "journal.txt").write_text("Title keyword", encoding="utf-8")

    # Cache text file to enrich deep scan
    ...
    assert (root_dir / "DB_Backup" / "NoteStore.sqlite").exists()
    assert (root_dir / "Spotlight_Backup").exists()

    query_out = list((root_dir / "Query_Output").glob("*.csv"))
    assert query_out

    verify_dirs = list(root_dir.glob("Verification*"))
    assert verify_dirs
    verify_out = list(verify_dirs[0].glob("*.csv"))
    assert verify_out

    report_paths = list(root_dir.glob("report_*.html"))
    assert report_paths
