...
SERVER_NAME = "io.github.xiaojiou176-open/notes-recover-mcp"
IMAGE_BASE = "ghcr.io/xiaojiou176-open/notes-recover"
...
def collect_docker_surface_errors(repo_root: Path) -> list[str]:
    ...
    local_image_tag = f"notes-recover:{project_version}"
...
def build_and_smoke(repo_root: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="notes-recover-docker-smoke-"):
        image_tag = f"notes-recover-smoke:{Path(repo_root).name}"
